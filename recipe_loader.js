...
const folder = './sp_recipes';

var files = fs.readdirSync(folder);
var text = fs.readFileSync(folder + '/' + files[0]).toString('utf-8');
...
for (att in recipe) {
  let ans = "";
  let t = typeof recipe[att];
  if (t == 'boolean') {
    sql_stmt = sql_stmt + att + ' BOOLEAN,\n';
  }
  else if (t == 'string') {
    sql_stmt = sql_stmt + att + ' TEXT,\n';
  }
  else if (t == 'number' ) {
    sql_stmt = sql_stmt + att + ' FLOAT,\n';
  }
  else {
    console.log(att, recipe[att]);
  }
}
